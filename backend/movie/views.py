...
from config.response import fail_response, success_response
...
class MovieRetrieveView(generics.GenericAPIView):
    ...
    @extend_schema(responses={200: success_response(MovieListOutputSerializer), 404: fail_response(MovieNotFound)})
    ...
    @extend_schema(responses={200: success_response(MovieDetailSerializer)})
...
class MovieDetailView(generics.GenericAPIView):
    ...
    @extend_schema(responses={200: success_response(MovieDetailSerializer), 404: fail_response(MovieNotFound)})
    ...
    @extend_schema(responses={200: success_response(data=None), 404: fail_response(MovieNotFound)})
